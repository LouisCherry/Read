...
class CurlAnimationProvider extends AnimationProvider {
	...
	private void drawInternalNoHack(Canvas canvas) {
		...
		final int oppositeX = (int)Math.abs(myWidth - cornerX);
		final int oppositeY = (int)Math.abs(myHeight - cornerY);
		final int x, y;
		if (myDirection.IsHorizontal) {
			...
			if (getMode().Auto) {
				...
				if (cornerY == 0) {
					y = (int)Math.max(1, (float)Math.min(myHeight / 2, myEndY));
				} else {
					y = (int)Math.max(myHeight / 2, (float)Math.min(myHeight - 1, myEndY));
				}
			}
		} else {
			y = myEndY;
		...
		if ((float)Math.abs(y1 - sY - cornerY) < myHeight) {
			myFgPath.lineTo(cornerX, oppositeY);
		}
		myFgPath.lineTo(oppositeX, oppositeY);
		if ((float)Math.abs(x1 - sX - cornerX) < myWidth) {
		...
		final float angle;
		if (cornerY == 0) {
			angle = -180 / 3.1416f * (float)(float)Math.atan2(x - cornerX, y - y1);
		} else {
			angle = 180 - 180 / 3.1416f * (float)(float)Math.atan2(x - cornerX, y - y1);
		}
	...
	protected void startAnimatedScrollingInternal(int speed) {
		mySpeedFactor = (float)(float)Math.pow(2.0, 0.25 * speed);
		mySpeed *= 1.5;
		doStep();
	}
	...
	void doStep() {
		...
		final int speed = (int)(float)Math.abs(mySpeed);
