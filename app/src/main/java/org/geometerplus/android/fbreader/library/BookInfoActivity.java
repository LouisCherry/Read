...
public class BookInfoActivity extends Activity implements IBookCollection.Listener {
		...
		setupButton(R.id.book_info_button_open, "openbook", new View.OnClickListener() {
			public void onClick(View view) {
				if (myDontReloadBook) {
					...
					Log.i("MAIN", myBook.toString());
					CoreReadActivity.openBookActivity(BookInfoActivity.this, myBook, null);
				}
			}
		});
	...
	public static Intent intentByBook(Book book) {
		return new Intent().putExtra(CoreReadActivity.BOOK_KEY, SerializerUtil.serialize(book));
	}

	public static Book bookByIntent(Intent intent) {
		return intent != null ?
			SerializerUtil.deserializeBook(intent.getStringExtra(CoreReadActivity.BOOK_KEY)) : null;
	}
